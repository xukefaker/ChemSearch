...
export function ManuscriptOutlineSheet({
  ...
  return (
    <AnimatePresence>
      {open ? (
        <motion.div
          className="absolute inset-0 z-[90] flex items-end justify-center bg-slate-950/18 p-4 backdrop-blur-[3px]"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={onClose}
        >
          <motion.section
            ...
            className="flex max-h-[78vh] w-full max-w-[30rem] flex-col overflow-hidden rounded-[1.9rem] border border-white/85 bg-white/96 shadow-[0_30px_80px_rgba(15,23,42,0.18)]"
          >
