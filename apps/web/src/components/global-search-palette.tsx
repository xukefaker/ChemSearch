    ...
    <AnimatePresence>
      {open ? (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[120] bg-[linear-gradient(180deg,rgba(15,23,42,0.18),rgba(15,23,42,0.32))] backdrop-blur-md"
          onClick={onClose}
        >
          <div className="flex min-h-full items-start justify-center px-4 pb-8 pt-[12vh] sm:px-8">
            <motion.div
              ...
              className="w-full max-w-[820px] overflow-hidden rounded-[2rem] border border-white/80 bg-white/94 shadow-[0_36px_100px_rgba(15,23,42,0.22),0_18px_44px_rgba(79,70,229,0.08)]"
            >
              <form
                onSubmit={(event) => {
                ...
                <div className="space-y-5 px-5 py-5 sm:px-6 sm:py-6">
                  {suggestionGroups.map((group) => (
                    <section key={group.label}>
                      <div className="mb-3 text-[0.68rem] font-bold uppercase tracking-[0.22em] text-slate-400">
                        {group.label}
                      </div>
