...
export function QuickPeekPanel({ paper, onClose, onOpenPaper }: QuickPeekPanelProps) {
  ...
  const { mounted, phase } = usePresence(Boolean(paper), 220);
  ...
  return (
    <div className='fixed inset-0 z-[120] flex items-end justify-center p-4 sm:items-center sm:p-8'>
      <div className='psa-overlay-backdrop absolute inset-0' data-state={phase} onClick={onClose} />
      <section
        role='dialog'
        aria-modal='true'
        ...
        className='psa-modal-surface relative flex max-h-[min(90vh,56rem)] w-full max-w-[64rem] flex-col overflow-hidden rounded-[2.2rem] border border-white/85 bg-white/96 shadow-[0_40px_120px_rgba(15,23,42,0.24)]'
      >
        <button
          ref={closeButtonRef}
          type='button'
          ...
          <div className='flex flex-col'>
            <div className='relative min-h-[16rem] border-b border-slate-100 bg-[radial-gradient(circle_at_top,rgba(99,102,241,0.12),transparent_40%),linear-gradient(180deg,#f8fafc_0%,#eef2f7_100%)]'>
              {paper.main_image_url ? (
                <>
                  <img src={paper.main_image_url} alt='Paper preview' className='max-h-[22rem] w-full object-cover object-top' />
            ...
            <div className='flex flex-col px-6 py-6 sm:px-8 sm:py-8'>
              ...
              {overview ? (
                <div className='mt-7 rounded-[1.5rem] border border-slate-200/90 bg-slate-50/90 px-5 py-5'>
                  ...
                  <p className='font-scholar mt-3 text-[1rem] leading-8 text-slate-700'>{overview.text}</p>
                </div>
              ) : null}
                ...
                <div className='space-y-6'>
                  {paper.structured_summary?.methodology ? (
                    <div className='rounded-[1.4rem] border border-slate-200/85 bg-white px-5 py-5'>
                      ...
                      <p className='mt-2 text-[0.95rem] leading-7 text-slate-600'>{paper.structured_summary.methodology}</p>
                    </div>
                  ) : null}
                </div>
                  ...
                  {benchmarks.length ? (
                    <div className='rounded-[1.4rem] border border-slate-200/85 bg-white px-5 py-5'>
                      ...
                      <div className='mt-3 flex flex-wrap gap-2'>
                        {benchmarks.map((item) => (
                          <span key={item} className='rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[0.72rem] font-semibold text-slate-600'>
                            {item}
                          </span>
                        ))}
                      </div>
