...
BACKEND_PATTERN="uvicorn paper_search_agent.api:app --host 127.0.0.1 --port 8000"
FRONTEND_NPM_PATTERN="npm run start -- --hostname 0.0.0.0 --port 3000"
...
backend_pid() {
  ...
  pgrep -f "$BACKEND_PATTERN" | head -n 1 || true
}
...
start_backend_process() {
  nohup env \
    ...
    "$ROOT_DIR/.venv/bin/python" -m uvicorn paper_search_agent.api:app --host 127.0.0.1 --port 8000 > "$BACKEND_LOG" 2>&1 < /dev/null &
  BACKEND_PID=$!
  echo "已启动后端，PID=$BACKEND_PID"
}

start_frontend_process() {
  nohup bash -lc "cd '$ROOT_DIR/apps/web' && exec ./node_modules/.bin/next start --hostname 0.0.0.0 --port 3000" > "$FRONTEND_LOG" 2>&1 < /dev/null &
  FRONTEND_PID=$!
...
print_status() {
  ...
  if curl -fsS http://127.0.0.1:3000/api/health >/dev/null 2>&1; then
    echo "健康检查 : 正常"
    echo "访问地址 : http://192.168.1.105:3000"
  else
    echo "健康检查 : 未通过"
  fi
...
start_demo() {
  ...
  if [[ -n "$existing_backend" ]]; then
    ...
    if wait_for_http http://127.0.0.1:8000/openapi.json 3; then
  ...
  if ! wait_for_http http://127.0.0.1:8000/openapi.json 20; then
  ...
  if [[ -n "$existing_frontend" ]]; then
    ...
    if wait_for_http http://127.0.0.1:3000/api/health 3; then
  ...
  if ! wait_for_http http://127.0.0.1:3000/api/health 30; then
