...
from chemsearch.mineru_pipeline import BatchItem, MinerUPipelineConfig, _build_batches, _progress_heartbeat, _run_batch
...
def test_run_batch_quiet_output_hides_noisy_parser_output(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr("chemsearch.mineru_pipeline.read_fn", lambda path: b"pdf")

    def _fake_parse(**kwargs) -> None:
        print("noisy stdout")
        print("noisy stderr", file=sys.stderr)

    monkeypatch.setattr("chemsearch.mineru_pipeline.do_parse", _fake_parse)
