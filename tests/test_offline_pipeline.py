...
from paper_search_agent.models import BuildIndexSummary, PaperRecord, ParseFailureRecord
from paper_search_agent.offline import (
...
def test_job_state_is_reinitialized_for_each_run(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("paper_search_agent.offline.time.time", lambda: next(timestamps))
    monkeypatch.setattr("paper_search_agent.offline.now_iso", lambda: next(iso_values))
...
def test_job_id_is_unique_even_within_same_second(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("paper_search_agent.offline.time.time_ns", lambda: next(values))
