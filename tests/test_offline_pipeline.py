...
from chemsearch.models import BuildIndexSummary, PaperRecord, ParseFailureRecord
from chemsearch.offline import (
...
def test_job_state_is_reinitialized_for_each_run(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("chemsearch.offline.time.time", lambda: next(timestamps))
    monkeypatch.setattr("chemsearch.offline.now_iso", lambda: next(iso_values))
...
def test_job_id_is_unique_even_within_same_second(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("chemsearch.offline.time.time_ns", lambda: next(values))
