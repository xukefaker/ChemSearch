...
from chemsearch.deep_chat.models import EvidencePack, HypothesisEvidencePack, RetrievedEvidence
...
def _enable_mocked_api(monkeypatch) -> None:
    ...
    monkeypatch.setattr("chemsearch.deep_chat.service.httpx.Client", _DeepChatClient)
...
def test_deep_chat_api_returns_409_on_upstream_failure(tmp_path: Path, monkeypatch) -> None:
    ...
    _enable_mocked_api(monkeypatch)
    monkeypatch.setattr("chemsearch.deep_chat.service.httpx.Client", _FailingClient)
    settings = _seed_fixture_data(tmp_path)
    monkeypatch.setenv("CHEMSEARCH_DATA_DIR", str(settings.data_dir))
    store = LocalStore(settings)
    _build_and_refresh_search_current(settings, store)
...
def test_deep_chat_api_downgrades_to_unsupported_when_verifier_rejects(tmp_path: Path, monkeypatch) -> None:
    ...
    _enable_mocked_api(monkeypatch)
    monkeypatch.setattr("chemsearch.deep_chat.service.httpx.Client", _VerifierRejectClient)
    settings = _seed_fixture_data(tmp_path)
    monkeypatch.setenv("CHEMSEARCH_DATA_DIR", str(settings.data_dir))
    store = LocalStore(settings)
    _build_and_refresh_search_current(settings, store)
...
def test_deep_chat_api_rejects_invalid_grounding_ids(tmp_path: Path, monkeypatch) -> None:
    ...
    _enable_mocked_api(monkeypatch)
    monkeypatch.setattr("chemsearch.deep_chat.service.httpx.Client", _InvalidGroundingClient)
    settings = _seed_fixture_data(tmp_path)
    monkeypatch.setenv("CHEMSEARCH_DATA_DIR", str(settings.data_dir))
    store = LocalStore(settings)
    _build_and_refresh_search_current(settings, store)
...
def test_deep_chat_api_retries_invalid_structured_output(tmp_path: Path, monkeypatch) -> None:
    ...
    _enable_mocked_api(monkeypatch)
    monkeypatch.setattr("chemsearch.deep_chat.service.httpx.Client", _RetryingClient)
    settings = _seed_fixture_data(tmp_path)
    monkeypatch.setenv("CHEMSEARCH_DATA_DIR", str(settings.data_dir))
    store = LocalStore(settings)
    _build_and_refresh_search_current(settings, store)
...
def test_deep_chat_api_rejects_invalid_fact_grounding_ids(tmp_path: Path, monkeypatch) -> None:
    ...
    _enable_mocked_api(monkeypatch)
    monkeypatch.setattr("chemsearch.deep_chat.service.httpx.Client", _InvalidFactClient)
    settings = _seed_fixture_data(tmp_path)
    monkeypatch.setenv("CHEMSEARCH_DATA_DIR", str(settings.data_dir))
    store = LocalStore(settings)
    _build_and_refresh_search_current(settings, store)
...
def test_deep_chat_api_rejects_cross_hypothesis_answer_grounding(tmp_path: Path, monkeypatch) -> None:
    ...
    _enable_mocked_api(monkeypatch)
    monkeypatch.setattr("chemsearch.deep_chat.service.httpx.Client", _CrossHypothesisGroundingClient)
    monkeypatch.setattr(
        "chemsearch.deep_chat.retriever.DeepChatRetriever.retrieve_for_hypotheses",
...
def test_deep_chat_api_uses_recent_user_correction_to_select_interpretation(tmp_path: Path, monkeypatch) -> None:
    ...
    _enable_mocked_api(monkeypatch)
    monkeypatch.setattr("chemsearch.deep_chat.service.httpx.Client", _CorrectionAwareClient)
...
def test_deep_chat_api_asks_for_clarification_when_verifier_sees_no_clear_winner(tmp_path: Path, monkeypatch) -> None:
    ...
    _enable_mocked_api(monkeypatch)
    monkeypatch.setattr("chemsearch.deep_chat.service.httpx.Client", _NoClearWinnerClient)
    settings = _seed_fixture_data(tmp_path)
    monkeypatch.setenv("CHEMSEARCH_DATA_DIR", str(settings.data_dir))
    store = LocalStore(settings)
    _build_and_refresh_search_current(settings, store)
