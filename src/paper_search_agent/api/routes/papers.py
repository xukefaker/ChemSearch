...
from ...models import ObjectRecord, PaperRecord, SectionRecord
...
def get_paper_viewer(request: Request, paper_id: str):
    with request.app.state.service_manager.acquire_services() as services:
        ...
        evidence_anchor_map: dict[str, str] = {}
        for spec in evidence_specs:
            ...
            for object_id in spec.get("object_ids", []):
                block_id = object_block_id_by_object_id.get(object_id)
                if block_id is not None:
                    break
            if block_id is None:
                section_id = str(spec.get("section_id") or "").strip()
                if section_id:
                    block_id = section_block_id_by_section_id.get(section_id)
            if block_id is not None:
                evidence_anchor_map[evidence_id] = block_id
        ...
        return JSONResponse(
            {
                ...
                "blocks": blocks,
                "evidence_anchor_map": evidence_anchor_map,
